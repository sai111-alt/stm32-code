...
void PWM_Init(void)
{
    ...
    // LED的是用OC1Init，也就是CH1通道，复用到PA0上，所以需要配置CH1和对应的GPIO
    // 配置CH1通道
    ...
    // 配置GPIOA的PA0,若重映射到PA15使用还需要配置AFIO
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO, ENABLE);  // 开启AFIO的时钟，若不重映射，则注释此行
    GPIO_PinRemapConfig(GPIO_PartialRemap1_TIM2, ENABLE); // 重映射函数，使CH1通道重映射到PA15，若不重映射，则注释此行
    GPIO_PinRemapConfig();
