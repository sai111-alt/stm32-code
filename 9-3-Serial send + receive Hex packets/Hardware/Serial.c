...
uint8_t Serial_TxPacket[4] = {0};
uint8_t Serial_RxPacket[4] = {0};
...
void USART1_IRQHandler(void)
{
    ...
    {
        /* 关于数据包的读取写入问题：这里是写入，那么另一个地方肯定需要对数据进行
        读出，当另一个地方读出后，原来的肯定要刷新为新的数据，这就存在一个问题
        就是读取和写入的速度，不管哪个快点哪个慢点，都可能出现问题，所以后面在其他
        模块使用该程序时要注意，如果出现问题，说不定就是读取写入的速度问题，这个问题
        的解决方法之一就是在下面的写入的程序中加上等我读完了你再写的程序，这样就可以规避速度问题 */
