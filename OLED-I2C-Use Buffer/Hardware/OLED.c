...
const ChineseCell_t OLED_CF16x16[] = {
    ...
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, /*"未命名文件"*/
};

uint8_t OLED_DisplayBuf[8][128]; /* 1KB的缓存数组，镜像GDDRAM */
...
{
    MyI2C_Start();
    MyI2C_SendByte(0x78); /* OLED Address Byte */
    MyI2C_ReceiveAck();
    MyI2C_SendByte(0x00); /* Control Byte，0非连续模式，0写命令，000000无关位 */
    MyI2C_ReceiveAck();
    MyI2C_SendByte(Command); /* Command Byte*/
    MyI2C_ReceiveAck();
    MYI2C_Stop();
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
    ...
    MyI2C_SendByte(0x78); /* OLED Address Byte */
    MyI2C_ReceiveAck();
    MyI2C_SendByte(0x40); /* Control Byte，0非连续模式，1写数据，000000无关位 */
    MyI2C_ReceiveAck();
    for (uint8_t i = 0; i < Count; i++)
    {
        MyI2C_SendByte(Data[i]); /* Write GDDRAM's Data Byte*/
        MyI2C_ReceiveAck();
    }
    MYI2C_Stop();
}
...
{
    OLED_WriteCommand(0x00 | (X & 0x0F));        /* 指定X坐标的低四位 */
    OLED_WriteCommand(0x10 | ((X & 0xF0) >> 4)); /* 指定X坐标的高四位 */
    OLED_WriteCommand(0xB0 | Page);              /* 指定Page的页数 */
}
